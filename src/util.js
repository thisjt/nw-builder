...
async function getReleaseInfo(version, platform, arch, cacheDir, manifestUrl) {
	...
	try {
		...
		releaseData = manifest.versions.find((release) => release.version === `v${version}`);
	} catch (e) {
		console.error('The version manifest does not exist/was not downloaded. Please try again in some time.');
	}
	return releaseData;
}
...
export const parse = async (options, pkg) => {
	...
	if (options.platform === 'linux') {
		...
		options.app.prefersNonDefaultGPU = options.app.prefersNonDefaultGPU ?? undefined;
	...
	if (options.platform === 'osx') {
		options.app.LSApplicationCategoryType = options.app.LSApplicationCategoryType ?? undefined;
		options.app.CFBundleIdentifier = options.app.CFBundleIdentifier ?? options.app.name;
		options.app.CFBundleName = options.app.CFBundleName ?? pkg.name;
		options.app.CFBundleDisplayName = options.app.CFBundleDisplayName ?? pkg.name;
		options.app.CFBundleSpokenName = options.app.CFBundleSpokenName ?? pkg.name;
		options.app.CFBundleShortVersionString = options.app.CFBundleVersion ?? pkg.version;
		options.app.CFBundleVersion = options.app.CFBundleShortVersionString ?? pkg.version;
		options.app.NSHumanReadableCopyright = options.app.NSHumanReadableCopyright ?? undefined;
	}
...
export const validate = async (options, releaseInfo) => {
	...
	if (!releaseInfo.flavors.includes(options.flavor)) {
		throw new Error(`${options.flavor} flavor is not supported by this download server.`);
	}
	if (options.platform && options.arch && !releaseInfo.files.includes(`${options.platform}-${options.arch}`)) {
		throw new Error(`Platform ${options.platform} and architecture ${options.arch} is not supported by this download server.`);
	}
	...
	if (options.logLevel !== 'error' && options.logLevel !== 'warn' && options.logLevel !== 'info' && options.logLevel !== 'debug') {
		throw new Error("Expected options.logLevel to be 'error', 'warn', 'info' or 'debug'. Got " + options.logLevel);
	}
	...
	if (typeof options.managedManifest !== 'boolean' && typeof options.managedManifest !== 'object' && typeof options.managedManifest !== 'string') {
		return new Error('Expected options.managedManifest to be a boolean, object or string. Got ' + typeof options.managedManifest);
	}
